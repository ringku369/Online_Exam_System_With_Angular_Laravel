...
@Component({
  selector: 'app-user-reportuserfundhistory',
  templateUrl: './user-reportuserfundhistory.component.html',
  styleUrls: ['./user-reportuserfundhistory.component.scss']
})
export class UserReportuserfundhistoryComponent implements OnInit {
  ...
  deposites:any = [];
  OnSubmitChild(){
    this.userUserService.dateWiseUserFundTrnHistory(this.form).subscribe(
      (response: any) => {
  ...
  // getUser(){
  //   this.userUserService.getUserForDD().subscribe(
  //     (response: any) => {
