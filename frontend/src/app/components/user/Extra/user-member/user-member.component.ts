...
import { UserMemberService } from 'src/app/services/user/user-member.service';
...
export class UserMemberComponent implements OnInit {
  ...
  public users:any = [];
  getDownlinkUser(){
    this.userMemberService.getDownlinkUser().subscribe(
      (response: any) => {
  ...
  submitForm():any {
    this.spinner.show();
    this.userMemberService.createUser(this.form).subscribe(
      (response: any) => {
