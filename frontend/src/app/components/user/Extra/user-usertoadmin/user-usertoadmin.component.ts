...
import { UserFundService } from 'src/app/services/user/user-fund.service';
...
@Component({
  selector: 'app-user-usertoadmin',
  templateUrl: './user-usertoadmin.component.html',
  styleUrls: ['./user-usertoadmin.component.scss']
})
export class UserUsertoadminComponent implements OnInit {
  ...
  postFundTrToAdminConfirm(data){
    this.spinner.show();
    this.userFundService.postFundTrToAdminConfirm(data).subscribe(
      (response: any) => {
  ...
  getAdminLinkUser(){
    this.userFundService.getAdminLinkUser().subscribe(
      (response: any) => {
        //console.log(response);
  ...
  onSubmit(): any {
    ...
    this.userFundService.postFundTrToAdmin(this.form).subscribe(
      (response: any) => {
