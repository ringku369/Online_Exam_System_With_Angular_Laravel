...
@Component({
  selector: 'app-admin-indirectsponsor',
  templateUrl: './admin-indirectsponsor.component.html',
  styleUrls: ['./admin-indirectsponsor.component.scss']
})
export class AdminIndirectsponsorComponent implements OnInit {
  ...
  getDownlinkUser(){
    this.adminUserService.getDownlinkUser().subscribe(
      (response: any) => {
  ...
  submitForm():any {
    this.spinner.show();
    this.adminUserService.postIndirectSponsor(this.form).subscribe(
      (response: any) => {
