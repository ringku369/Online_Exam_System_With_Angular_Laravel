...
import { AdminMemberService } from 'src/app/services/admin/admin-member.service';
...
export class AdminMemberComponent implements OnInit {
  ...
  public users:any = [];
  getDownlinkUser(){
    this.adminMemberService.getDownlinkUser().subscribe(
      (response: any) => {
  ...
  submitForm():any {
    this.spinner.show();
    this.adminMemberService.createUser(this.form).subscribe(
      (response: any) => {
