...
@Component({
  selector: 'app-admin-userbanarymtreport',
  templateUrl: './admin-userbanarymtreport.component.html',
  styleUrls: ['./admin-userbanarymtreport.component.scss']
})
export class AdminUserbanarymtreportComponent implements OnInit {
  ...
  OnSubmitChild(){
    this.spinner.show();
    this.sum = 0;
    this.adminReportService.binaryMatchingReport(this.form).subscribe(
      (response: any) => {
        this.users = response;
        //console.log(response);
        this.spinner.hide();
