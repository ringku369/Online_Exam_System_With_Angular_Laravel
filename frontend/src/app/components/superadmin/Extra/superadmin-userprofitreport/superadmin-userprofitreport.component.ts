...
@Component({
  selector: 'app-superadmin-userprofitreport',
  templateUrl: './superadmin-userprofitreport.component.html',
  styleUrls: ['./superadmin-userprofitreport.component.scss']
})
export class SuperadminUserprofitreportComponent implements OnInit {
  ...
  OnSubmitChild(){
    this.sum = 0;
    this.superadminReportService.dateWiseSelfProfit(this.form).subscribe(
      (response: any) => {
        this.users = response;
        //console.log(response);
        this.spinner.hide();
