...
@Component({
  selector: 'app-superadmin-tree',
  templateUrl: './superadmin-tree.component.html',
  styleUrls: ['./superadmin-tree.component.scss']
})
export class SuperadminTreeComponent implements OnInit {
  ...
  getDownlinkallUser(){
    ...
    this.superadminTreeService.getDownlinkallUser().subscribe(
      (response: any) => {
        this.i = true;
  ...
  sendMailToUser(){
    this.spinner.show();
    this.superadminTreeService.sendMailToUser().subscribe(
      (response: any) => {
        //console.log(response);
