...
@Component({
  selector: 'app-superadmin-usertransactionreport',
  templateUrl: './superadmin-usertransactionreport.component.html',
  styleUrls: ['./superadmin-usertransactionreport.component.scss']
})
export class SuperadminUsertransactionreportComponent implements OnInit {
  ...
  OnSubmitChild(){
    this.sum = 0;
    this.superadminReportService.dateWiseSelfTransaction(this.form).subscribe(
      (response: any) => {
        this.users = response;
        //console.log(response);
        this.spinner.hide();
